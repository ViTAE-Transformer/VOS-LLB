...
import ltr.models.joint.combiner as seg_combiner
...
class JOINTNet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_masks, test_masks, num_refinement_iter=2):
        ...
        train_masks = torch.unsqueeze(train_masks, 2)
        filter_img = train_imgs * train_masks
        few_shot_label_tm, few_shot_label_tr, few_shot_sw_tm = self.label_encoder(filter_img, None)
        ...
        for i in range(num_test_frames):
            ...
            mask_encoding_cmb = self.combiner(mask_encoding_pred_last_iter, mask_encoding_transfered)
            ...
            test_img_it = test_imgs[i:i + 1, ...]
            mask_pred_prob = torch.unsqueeze(mask_pred_prob, 2)
            filter_test_img = test_img_it * mask_pred_prob
            few_shot_label_tm, few_shot_label_tr, few_shot_sw_tm = self.label_encoder(filter_test_img)
    ...
    def segment_target(self, target_filter, test_feat_tm, test_feat, mask_encoding_transfered, frame_num):
        ...
        mask_encoding_cmb = self.combiner(mask_encoding_pred, mask_encoding_transfered)  # * mask_encoding_pred_tmp #+ mask_encoding_transfered
...
@model_constructor
def steepest_descent_resnet50(filter_size=1, num_filters=32, optim_iter=3, optim_init_reg=0.01,
                              backbone_pretrained=False, clf_feat_blocks=0,
                              clf_feat_norm=True, final_conv=True,
                              out_feature_dim=512,
    ...
    label_encoder_dims = layer_channels[target_model_input_layer]
    # Few-shot label generator and weight predictor
    label_encoder = seg_label_encoder.ResidualDS16SW(layer_dims= [num_filters,],
                                                     frozen_backbone_layers=frozen_backbone_layers,use_bn=use_bn_in_label_enc)
    ...
    combiner = seg_combiner.Combiner(num_filters)
    ...
    net = JOINTNet(feature_extractor=backbone_net, transformer=transformer, target_model=target_model,
                  combiner=combiner, decoder=decoder, label_encoder=label_encoder,
