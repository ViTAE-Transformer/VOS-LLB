...
class NormalCell(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.class_token:
            n = n - 1
            # wh = int(math.sqrt(n))
            convX = self.drop_path(self.PCM(x[:, 1:, :].view(b, H, W, c).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous().view(b, n, c))
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x[:, 1:] = x[:, 1:] + convX
        else:
            # wh = int(math.sqrt(n))
            convX = self.drop_path(self.PCM(x.view(b, H, W, c).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous().view(b, n, c))
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x + convX
