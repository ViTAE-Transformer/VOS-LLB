...
def run(settings):
    ...
    # Datasets
    coco_set = MSCOCO(version="2017", split='train')
    syn_data_train = SyntheticVideo(base_image_dataset=coco_set)
    ...
    dataset_train = sampler.SegSampler([syn_data_train], [1],
    ...
    vitae_path = os.path.join(settings.env.pretrained_networks, 'ViTAE-6M.pth.tar')
    load_checkpoint(net.label_encoder.label_backbone,vitae_path, use_ema=True, strict=True)
    ...
    optimizer = optim.Adam([{'params': actor.net.target_model.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.label_encoder.parameters(), 'lr': 1e-4},
                            {'params': actor.net.transformer.parameters(), 'lr': 2e-5},
                            {'params': actor.net.combiner.parameters(), 'lr': 1e-4}],
                           lr=2e-4)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30, ], gamma=0.2)
