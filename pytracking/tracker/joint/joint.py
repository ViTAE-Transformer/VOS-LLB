...
class JOINT(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        init_backbone_feat, filter_img = self.generate_init_samples(im, init_mask)

        # Initialize target model
        self.init_target_model(init_backbone_feat, filter_img)
    ...
    def track(self, image, info: dict = None) -> dict:
            ...
            if self.params.get('update_target_model', True):
                prev_segmentation_prob_crop, _ = sample_patch(prev_segmentation_prob_im, self.prev_pos,
                                                              ...
                                                              max_scale_change=self.params.get(
                                                                  'patch_max_scale_change'),
                                                              is_mask=True)

                # Update the target model
                pre_filter_img = self.pre_test_img.cpu() * prev_segmentation_prob_crop.clone()
                self.update_target_model(self.prev_test_x_bb, pre_filter_img)
        ...
        # segmentation_prob_np = segmentation_prob_im.cpu().numpy()
        # index = np.where(segmentation_prob_np==1)
        # ************************************************************************ #
    ...
    def segment_target(self, sample_bb_feat, sample_x):
        with torch.no_grad():
            mask_encoding_transferred = self.net.transformer(sample_bb_feat.unsqueeze(1), self.train_feat_bb_it,
                                                             train_mask_enc=self.few_shot_label_tr_it)
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scale, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scale.unsqueeze(0), sz,
                                                           ...
                                                           max_scale_change=self.params.get('patch_max_scale_change',
                                                                                            None))
        with torch.no_grad():
            backbone_feat, im_patches_precessed = self.net.extract_backbone(im_patches)
        return backbone_feat, patch_coords[0], im_patches_precessed
    ...
    def generate_init_samples(self, im: torch.Tensor, init_mask):
        ...
        with torch.no_grad():
            init_backbone_feat, im_patches_pre = self.net.extract_backbone(im_patches)

        init_masks = init_masks.to(self.params.device)
        filter_img = im_patches_pre * init_masks
        return init_backbone_feat, filter_img
    ...
    def init_target_model(self, init_backbone_feat, filter_img):
        ...
        with torch.no_grad():
            ...
            filter_img_ext = filter_img.unsqueeze(1)
            few_shot_label_tm, few_shot_label_tr, few_shot_sw = self.net.label_encoder(filter_img_ext, x.unsqueeze(1))
        ...
        # Init memory
        if self.params.get('update_target_model', True):
            self.init_memory(TensorList([x_bb]), masks=filter_img.view(-1, *filter_img.shape[-3:]))

    def update_target_model(self, train_x_bb, filter_img, learning_rate=None):
        ...
        if self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory(TensorList([train_x_bb]), filter_img, learning_rate)
            memory_updated = True
        ...
        if num_iter > 0:
            ...
            with torch.no_grad():
                ...
                filter_imgs = masks.unsqueeze(1)
                few_shot_label_tm, few_shot_label_tr, few_shot_sw = self.net.label_encoder(filter_imgs, samples.unsqueeze(1))
